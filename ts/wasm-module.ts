 ...
 *
 * The build emits `dist/rnp.js` as ESM (Emscripten EXPORT_ES6=1). We locate
...
export interface InitOptions {
   ...
   * resolves `${import.meta.url}` → `./rnp.wasm`. Override for CDNs, bundlers
   * with custom asset paths, or for embedding the wasm as base64.
   */
 ...
 *
 * The factory lives at `../dist/rnp.js` (ESM via EXPORT_ES6=1). The actual
...
export async function loadModule(opts: InitOptions = {}): Promise<RnpModule> {
  // The dynamic import path resolves to dist/rnp.js, which is Emscripten-
  ...
  const factory = ((await import("../dist/rnp.js")).default as (opts: Record<string, unknown>) => Promise<RnpModule>);
